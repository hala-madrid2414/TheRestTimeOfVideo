# README and Script Improvements Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 优化 README 结构以突出快速开始流程，并在构建脚本执行完毕后在终端输出醒目的安装引导信息。

**Architecture:** 
1. 修改 `README.md`，将通用的安装流程提取为“快速开始”章节，简化原有的“使用者指南”和“开发者指南”。
2. 在 `scripts/check-dist.mjs` 中添加控制台输出逻辑，利用 ASCII 字符构建醒目的引导框。

**Tech Stack:** Markdown, Node.js (mjs)

---

### Task 1: Update scripts/check-dist.mjs

**Files:**
- Modify: `d:\TheRestTimeOfVideo\scripts\check-dist.mjs`

**Step 1: Write the implementation**

在 `scripts/check-dist.mjs` 的 `main` 函数末尾（`console.log('dist check passed');` 之后）添加以下引导信息输出：
...
Expected: 成功输出带有边框的引导信息。

**Step 3: Commit**

```bash
git add scripts/check-dist.mjs
git commit -m "feat(scripts): add installation guide output after build"
```

### Task 2: Update README.md Structure

**Files:**
- Modify: `d:\TheRestTimeOfVideo\README.md`

**Step 1: Write the implementation**

修改 `README.md`，重构为以下结构：
...
(Visual verification only - read file to ensure changes look correct)

**Step 3: Commit**

```bash
git add README.md
git commit -m "docs: restructure README to extract Quick Start section"
```
